...
export default function NewsComment({ userData, news }) {
  ...
  return (
    <div className="comments-container mt-5">
      <div>
        {comments.length != 0 ? (
          <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg p-4 comment-form">
            <h2 className="text-lg font-semibold mb-3">Comments</h2>
            <div className="space-y-3">
              {comments.map((comment, index) => (
                <div
                  key={index}
                  className="bg-gray-100 p-3 rounded-lg shadow comment-card"
                >
                  <p className="ayuda text-lg text-black">
                    <i>
                      {usersList.find((u) => u.id === comment.user_id)?.name}
                    </i>
                    <span className="text-xs text-gray-500 mx-5 comment-date">
                      🗓 {comment.date}
                    </span>
                  </p>
                  <p className="text-sm text-gray-700 comment-score">
                    Score: {comment.rate}
                  </p>
                  <p className="text-sm text-gray-700 comment-content">
                    {comment.content}
                  </p>
                </div>
              ))}
      ...
      <div className="mx-auto bg-white shadow-lg rounded-lg p-4 mt-10 mb-6 comment-form">
